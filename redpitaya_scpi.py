...
class scpi(object):
    ...
    def board_get_date_time(self) -> str:
        """
    ...
    ### ANALOG IO ###
    def analog_get_data(self) -> np.ndarray:
        """
        Return data from all 4 slow analog inputs as a numpy array.
        """
        data = np.array(
            [self.txrx_txt(f"ANALOG:PIN? AIN{i}") for i in range(4)], dtype=float
        )
        self.check_error()
    ...
    def daisy_get_settings(self) -> List[str | None]:
        """
    ...
    def pll_enable(self, siglab: bool = False) -> None:
        """
    ...
    def pll_disable(self, siglab: bool = False) -> None:
        """
    ...
    def pll_get_state(self, siglab: bool = False) -> List[str | None]:
        """
    ...
    def gen_get_burst_settings(self, chan: int) -> List[str | None]:
        ...
        print("after settings recieved")
        self.check_error()
        print("after check error")
        # ? Remove prints? Repace with logging?
    ...
    def gen_sweep_set(
        ...
        self._validate_sweep_params(
            chan, start_freq, stop_freq, time_us, mode, direction, sdrlab
        )
    ...
    def _validate_gen_set_params(
        ...
        assert freq_down_lim <= freq <= freq_up_lim, (
            f"Frequency is out of range {freq_down_lim, freq_up_lim} Hz"
        )
        assert abs(volt) <= volt_lim, (
            f"Amplitude is out of range {-volt_lim, volt_lim} V"
        )
        if offset is not None:
            assert abs(offset) <= offs_lim, (
                f"Offset is out of range {-offs_lim, offs_lim} V"
            )
        if dcyc is not None:
            assert 0 <= dcyc <= 1, "Duty Cycle is out of range {0, 1}"
        if phase is not None:
            assert abs(phase) <= phase_lim, (
                f"Phase is out of range {-phase_lim, phase_lim} deg"
            )
        if data is not None:
            assert data.shape[0] <= buff_size, (
                f"Data array is too long. Max length is {buff_size}"
            )
        if trig_sour is not None:
            assert trig_sour.value in trigger_list, (
                f"{trig_sour.value} is not a defined trigger source"
            )
        if ext_trig_deb_us is not None:
            assert ext_trig_deb_us >= 1, (
                f"External trigger debounce filter value {ext_trig_deb_us} is out of range. The minimal value is 1 microsecond"
            )
        if ext_trig_lev is not None:
            assert abs(ext_trig_lev) <= volt_lim, (
                f"External trigger level is out of range {-volt_lim, volt_lim} V"
            )
        if load is not None:
            assert load.value in load_list, (
                f"{load.value} is not a defined load for SIGNALlab 250-12"
            )
        assert not (siglab and sdrlab), (
            "Please select only one board option. 'siglab' and 'sdrlab' cannot be true at the same time."
        )
    ...
    def _validate_burst_params(
        ...
        assert abs(last_val) <= volt_lim, (
            f"Last value is out of range {-volt_lim, volt_lim} V"
        )
        assert abs(init_val) <= volt_lim, (
            f"Init value is out of range {-volt_lim, volt_lim} V"
        )
    ...
    def _validate_sweep_params(
        ...
        self._validate_channel(chan)
        assert freq_down_lim < start_freq <= freq_up_lim, (
            f"Start frequency is out of range {freq_down_lim, freq_up_lim} Hz"
        )
        assert freq_down_lim < stop_freq <= freq_up_lim, (
            f"Stop frequency is out of range {freq_down_lim, freq_up_lim} Hz"
        )
        ...
        assert direction in SweepDirection, (
            f"{direction.value} is not a defined sweep direction"
        )
    ...
    def acq_set(
        ...
        """
        self._validate_acq_set_params(
            dec, units, data_format, gain, coupling, siglab, input4
        )
    ...
    def acq_get_settings(
        self, siglab: bool = False, input4: bool = False
    ) -> List[str | None]:
        """
        ...
        if input4:
            print(
                f"Gain CH1/CH2/CH3/CH4: {settings[5]}, {settings[6]}, {settings[7]}, {settings[8]}"
            )
        else:
    ...
    def acq_trig_set(
        ...
        true (siglab, input4). This will change the available range of input parameters.
        """
        self._validate_acq_trig_params(
            trig_lvl,
            trig_delay,
            trig_hyst,
            ext_trig_deb_us,
    ...
    def acq_trig_ext_hyst_set(
        ...
        """
        self._validate_acq_trig_ext_hyst_params(
            trig_hyst, ext_trig_deb_us, ext_trig_lvl, siglab
        )
    ...
    def acq_set_units_format(
        self, units: Optional[Units] = None, data_format: Optional[DataFormat] = None
    ) -> None:
        """
    ...
    def acq_split_trig_set(
        ...
        This will change the available range of input parameters.
        """
        self._validate_acq_split_trig_params(
            chan, trig_lvl, trig_delay, trig_delay_ns, input4
        )
    ...
    def acq_data(
        ...
        """
        self._validate_acq_data_params(
            chan, start, end, num_samples, old, last, trig_pos, input4
        )
    ...
    def _validate_acq_set_params(
        ...
        assert (dec not in dec_fact_list) and (16 <= dec <= 65536), (
            "Decimation factor out of range [1,2,4,8,16,17,18,...,65536]"
        )
        if units is not None:
            assert units.value in units_list, f"{units.value} is not a defined unit"
        if data_format is not None:
            assert data_format.value in format_list, (
                f"{data_format.value} is not a defined format"
            )
        if gain is not None:
            for g in gain:
    ...
    def _validate_units_format(
        self, units: Optional[Units], data_format: Optional[DataFormat]
    ) -> None:
        """
    ...
    def _validate_acq_trig_params(
        ...
        trig_lvl_lim = (
            20.0
            if any(
                self.txrx_txt(f"ACQ:SOUR{i + 1}:GAIN?").upper() == "HV"
        ...
        assert abs(trig_lvl) <= trig_lvl_lim, (
            f"Trigger level out of range {-trig_lvl_lim, trig_lvl_lim} V"
        )
        assert trig_delay >= 0, "Trigger delay cannot be less than 0"
        if trig_hyst is not None:
            assert trig_hyst >= 0, "Trigger hysteresis cannot be negative"
        if siglab and ext_trig_lvl is not None:
            assert abs(ext_trig_lvl) <= ext_trig_lvl_limit, (
                f"External trigger level out of range {-ext_trig_lvl_limit, ext_trig_lvl_limit} V"
            )
        if ext_trig_deb_us is not None:
            assert ext_trig_deb_us >= 1, (
                "External trigger debounce filter value is out of range. The minimal value is 1 microsecond"
            )
        assert not (siglab and input4), (
    ...
    def _validate_acq_split_params(
        ...
        assert chan <= n, (
            f"Channel {chan} out of range for the current Red Pitaya board"
        )
        assert (dec not in dec_fact_list) and (16 <= dec <= 65536), (
            "Decimation factor out of range [1,2,4,8,16,17,18,...,65536]"
        )
        ...
        if siglab and coupling is not None:
            assert coupling.value in coupling_list, (
                f"{coupling.value} is not a defined coupling"
            )
        assert not (siglab and input4), (
            "Please select only one board option. 'siglab' and 'input4' cannot be true at the same time."
        )
    ...
    def _validate_acq_split_trig_params(
        self, chan: int, trig_lvl: float, trig_delay: int, input4: bool
    ) -> None:
        """
        ...
        assert abs(trig_lvl) <= trig_lvl_lim, (
            f"Trigger level out of range {-trig_lvl_lim, trig_lvl_lim} V for gain {gain_lvl}"
        )
        assert trig_delay >= 0, "Trigger delay cannot be less than 0"
    ...
    def _validate_acq_data_params(
        ...
        assert not (old and last), (
            "Please select only one. 'old' and 'last' cannot be True at the same time."
        )
        if start is not None:
            assert low_lim <= start <= up_lim, (
                f"Start position out of range {low_lim, up_lim}"
            )
        if end is not None:
            assert low_lim <= end <= up_lim, (
                f"End position out of range {low_lim, up_lim}"
            )
        if num_samples is not None:
            assert low_lim <= num_samples <= up_lim, (
                f"Sample number out of range {low_lim, up_lim}"
            )
            if trig_pos is not None:
                assert trig_pos in DataTriggerPosition, (
                    f"Trigger position value {trig_pos} is not defined"
                )
                if trig_pos == DataTriggerPosition.PRE_POST_TRIG:
                    assert num_samples * 2 + 1 <= up_lim, (
                        f"Sample number is too big for {trig_pos.value} setting. This mode returns num_samples*2 +1 data samples."
                    )
    ...
    def uart_write_string(self, string: str, word_length: bool = False) -> None:
        """
    ...
    def _validate_uart_params(
        self, speed: int, bits: UartBits, parity: UartParity, stop: int, timeout: int
    ) -> None:
        """
    ...
    def spi_set(
        ...
        try:
            ...
            raise ValueError(
                f"{speed} is out of range [{speed_min_limit},{speed_max_limit}]."
            ) from speed_err
